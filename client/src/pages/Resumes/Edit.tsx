...
export default function EditResume() {
    ...
    useEffect(() => {
        api.get(`/resumes/${resumeId}`).then((r: AxiosResponse<{
            message: string,
            data: {
                resume: IResume
            }
        }>) => {
            const resumeData = r.data.data.resume

            const modifiedWorkExperience = []

            for (let i = 0; i < resumeData.workExperience.length; i++) {
                const we = resumeData.workExperience[i];

                modifiedWorkExperience.push({
                    ...we,
                    currentlyWorking: !we.endDate
                })
            }
    ...
    const handleSubmit = () => {
        ...
        api.patch(`/resumes/${resumeId}`, inputs).then(() => {
            setLoading(false)
